...
{
    public sealed class KerberosAuthenticator : IAuthenticator
    {
        private readonly IGssapiProvider _gss;
        ...
        public KerberosAuthenticator(IGssapiProvider gssapi, KerberosOptions options, IMetricsManager? metrics = null, string componentName = "Silica.Authentication.Kerberos", IPrincipalMapper? principalMapper = null)
        {
            _gss = gssapi ?? throw new ArgumentNullException(nameof(gssapi));
            _options = options ?? new KerberosOptions();
            _metrics = metrics;
            _componentName = string.IsNullOrWhiteSpace(componentName) ? "Silica.Authentication.Kerberos" : componentName;
        ...
        public async System.Threading.Tasks.Task<IAuthenticationResult> AuthenticateAsync(AuthenticationContext context, CancellationToken cancellationToken = default)
        {
            ...
            AuthenticationDiagnostics.EmitDebug(_componentName, "Authenticate", "attempt_kerberos", null, baseTags, allowDebug: true);
            ...
            if (context == null)
            {
                ...
                return new KerberosAuthenticationResult { Succeeded = false, FailureReason = AuthenticationFailureReasons.InvalidRequest };
            }
            if (!_gss.IsAvailable)
            {
                var ex = new Silica.Authentication.AuthenticationProviderUnavailableException("Kerberos");
            ...
            if (context.TokenBytes is not null && context.TokenBytes.Length > 0)
            {
                if (_options.MaxTokenBytes > 0 && context.TokenBytes.Length > _options.MaxTokenBytes)
                {
                    ...
                    var elapsedFail = StopwatchUtil.GetElapsedMilliseconds(start);
                    AuthenticationMetrics.RecordLatency(_metrics, elapsedFail, Name);
                    return new KerberosAuthenticationResult { Succeeded = false, FailureReason = AuthenticationFailureReasons.TokenTooLong };
                }
            ...
            else if (!string.IsNullOrWhiteSpace(context.Token))
            {
                ...
                catch
                {
                    ...
                    var elapsedFail = StopwatchUtil.GetElapsedMilliseconds(start);
                    AuthenticationMetrics.RecordLatency(_metrics, elapsedFail, Name);
                    return new KerberosAuthenticationResult { Succeeded = false, FailureReason = AuthenticationFailureReasons.InvalidTokenBase64 };
                }
                // Enforce maximum length on decoded bytes as well (parity with NTLM).
            ...
            else
            {
                ...
                var elapsedFail = StopwatchUtil.GetElapsedMilliseconds(start);
                AuthenticationMetrics.RecordLatency(_metrics, elapsedFail, Name);
                return new KerberosAuthenticationResult { Succeeded = false, FailureReason = AuthenticationFailureReasons.MissingToken };
            }
            string? principal = null;
            try
            {
                principal = await _gss.ValidateTokenAsync(tokenBytes, _options.ServicePrincipal, cancellationToken).ConfigureAwait(false);
            }
            ...
            catch (TimeoutException tex)
            {
                ...
                throw new Silica.Authentication.AuthenticationProviderUnavailableException("Kerberos");
            }
            catch (Exception ex)
            {
            ...
            if (principal == null)
            {
                ...
                AuthenticationDiagnostics.Emit(_componentName, "Authenticate", "warn", "invalid_kerberos_token", null, baseTags);
                await ApplyFailureDelayAsync(cancellationToken).ConfigureAwait(false);
                var elapsedFail = StopwatchUtil.GetElapsedMilliseconds(start);
                AuthenticationMetrics.RecordLatency(_metrics, elapsedFail, Name);
                return new KerberosAuthenticationResult { Succeeded = false, FailureReason = AuthenticationFailureReasons.InvalidToken };
            }
            ...
            AuthenticationDiagnostics.Emit(_componentName, "Authenticate", "info", "kerberos_auth_succeeded", null, okTags);
            return new KerberosAuthenticationResult { Succeeded = true, Principal = principal, Roles = roles };
        }
    }
}
