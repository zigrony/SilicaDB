...
namespace Silica.Authentication.Diagnostics
{
    /// <summary>
    /// Centralized diagnostics for Silica.Authentication.
    /// Mirrors ConcurrencyDiagnostics: normalized levels, reserved tags, verbosity gate.
    /// </summary>
    internal static class AuthenticationDiagnostics
    {
        ...
        internal static void ReloadFromEnvironment()
        {
            ...
            {
                string? v = null;
                try { v = Environment.GetEnvironmentVariable("SILICA_AUTHENTICATION_VERBOSE"); } catch { }
        ...
        internal static void Emit(string component, string operation, string level, string message, Exception? ex = null, IReadOnlyDictionary<string, string>? more = null)
        {
            ...
            string componentName = "Silica.Authentication";
            string operationName = operation ?? string.Empty;
            try
            {
                // Stamp minimal identity
                componentName = string.IsNullOrWhiteSpace(component) ? "Silica.Authentication" : component;
            ...
            try
            {
                ...
                else if (string.Equals(lvl, "FATAL", StringComparison.OrdinalIgnoreCase)) lvl = "error"; // normalize
