// AuthenticationTestHarnessNtlm.cs
// Lightweight harness to exercise Silica.Authentication.Ntlm.NtlmAuthenticator
// - Exercises success/failure/malformed/missing-token/provider-unavailable paths
// - Uses DummyNtlmSecurityProvider for deterministic behavior
// - Uses a simple DummyMetricsManager so metric helpers are exercised
...
using Silica.Authentication.Ntlm;
using Silica.Authentication.Diagnostics;
using Silica.DiagnosticsCore.Metrics;

namespace Silica.Authentication.Tests
{
    public static class AuthenticationTestHarnessNtlm
    {
        public static async Task Run()
        {
            Console.WriteLine("=== NTLM Authentication Test Harness ===");

            try { AuthenticationDiagnostics.SetVerbose(true); } catch { }

            IMetricsManager metrics = new DummyMetricsManager();

            var provider = new DummyNtlmSecurityProvider();
            var options = new NtlmOptions();
            var auth = new NtlmAuthenticator(provider, options, metrics: metrics, componentName: "Silica.Authentication.Ntlm");

            await RunTest("[NTLM] Success with VALID token", () => TestSuccess(auth));
            await RunTest("[NTLM] Failure with BAD token", () => TestInvalidToken(auth));
            await RunTest("[NTLM] Missing token", () => TestMissingToken(auth));
            await RunTest("[NTLM] Malformed base64 token", () => TestMalformedBase64(auth));
            await RunTest("[NTLM] Provider unavailable", () => TestProviderUnavailable());

            Console.WriteLine("=== NTLM Authentication Test Harness Complete ===");
        }
        ...
        private static async Task TestSuccess(NtlmAuthenticator auth)
        {
            ...
            if (res.Principal != "TESTDOMAIN\\Alice") throw new InvalidOperationException("Unexpected principal returned.");
        }

        private static async Task TestInvalidToken(NtlmAuthenticator auth)
        {
        ...
        private static async Task TestMissingToken(NtlmAuthenticator auth)
        {
        ...
        private static async Task TestMalformedBase64(NtlmAuthenticator auth)
        {
        ...
        {
            var unavailableProvider = new UnavailableNtlmSecurityProvider();
            var options = new NtlmOptions();
            var metrics = new DummyMetricsManager();
            var auth = new NtlmAuthenticator(unavailableProvider, options, metrics: metrics, componentName: "Silica.Authentication.Ntlm");
        ...
        private sealed class UnavailableNtlmSecurityProvider : INtlmSecurityProvider
        {
